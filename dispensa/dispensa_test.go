...
Il programma dovrà essere dotato delle seguenti:
  ...
  - una funzione AggiornaDispensa(dispensa map[string]float64, filename string) bool
    ...
    In particolare il file conterrà gli acquisti nel formato "approv,<prodotto>,<peso>" e i consumi nel
    formato "uso,<prodotto>,<peso>", dove le quantità (pesi) sono sempre espresse in Kg.
    (Non vanno fatti controlli sul formato dei dati, si può assumere che siano sempre esattamente come nel file di esempio).
    Se il file non esiste o non è leggibile, la funzione deve terminare subito restituendo 'false'.
    Nel caso in cui un prodotto dovesse "andare in negativo"
    durante l'aggiornamento della dispensa, la funzione dovrà terminare subito e restituire 'false', mentre restituirà
    'true' in caso contrario.

  - una funzione Rimanenza(dispensa map[string]float64, alimento string) float64
    che, data una dispensa opportunamente popolata di provviste, restituisce la rimanenza in Kg dell'alimento passato
    come parametro; restituisce 0 se il prodotto non è presente in lista.
  ...
  - una funzione main()
    ...
    In caso di presenza di argomenti oltre al nome del file, il programma stampa poi la rimanenza degli alimenti passati come
    argomenti successivi.
    In caso di mancanza di argomenti oltre al nome del file, il programma deve stampare solo "file corretto" se nessun prodotto
    è andato in negativo, e "file non corretto" in caso contrario.
